  ...
  useEffect(() => {
    const validateStoredAdminSession = async () => {
      try {
        ...
        // Check if this is an offline admin (bypass Supabase session validation)
        const isOfflineAdmin = localStorage.getItem('isOfflineAdmin') === 'true';
        if (isOfflineAdmin) {
        ...
        console.warn('No active supabase session found; clearing stored admin state');
