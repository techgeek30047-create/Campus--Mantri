  ...
  admins: [
    {
      ...
      username: 'admin',
      name: 'Offline Admin',
      email: 'admin@campusmantri.local',
      password: 'admin123',
      is_super: true,
...
export const supabase = (realSupabase || createOfflineSupabaseClient()) as any;
...
export const testSupabaseConnection = async (): Promise<SupabaseConnectionResult> => {
  try {
    const rows = readTable('campus_mantris');
    return { connected: true, error: rows ? undefined : 'Unable to access offline data' };
  } catch (error) {
